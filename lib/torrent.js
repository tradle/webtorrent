...
var holepunch = require('holepunch');
...
Torrent.prototype.addPeer = function (peer) {
  ...
  if (typeof peer === 'string'
    ...
    var ipPort = addrToIPPort(peer)
    if (ipPort[0] === '127.0.0.1') addPeer()
    else {
      holepunch({
        port: ipPort[1],
        host: ipPort[0],
        localPort: this.client.torrentPort
      }, function(err, socket) {
        if (err) console.error('hole punching error', err)
        else socket.close()
